class Dashboard {
    ...
    handleSearch(e) {
      ...
      const filtered = this.dashboard.transactions.filter((t) =>
    ...
    updateUI(filtered = this.dashboard.transactions) {
      ...
      filtered.forEach((trans) => {
        ...
        tr.innerHTML = `
          <td style="text-transform:uppercase;color:${isDeposit ? "rgb(0, 255, 0)" : "red"}">${trans.type}</td>
          <td>${trans.amount.toFixed(2)} $</td>
          <td>${trans.date}</td>
          <td><i class="ri-corner-down-${isDeposit ? "left" : "right"}-fill" style="color:${isDeposit ? "rgb(0, 255, 0)" : "rgb(253, 0, 0)"}"></i></td>
        `;
  ...
  // DOM tayyor bo‘lganda dashboard ishga tushadi
